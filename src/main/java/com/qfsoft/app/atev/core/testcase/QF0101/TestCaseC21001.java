...
public class TestCaseC21001 extends TestCase {
	
	// 数据抓取: 获取基金数据
	public TestCaseC21001(TestExecDetail ted) {
		super(ted);
	}
	...
	public JsonObject exec() throws Exception {
		for (int n = 0; n < 10; n++) {
			String sql2 = "SELECT * FROM t_invest_fund WHERE 1=1 AND (update_date IS NULL OR update_date<CURDATE())";
			DefaultTableModel dtm2 = DatabaseMethod.query(ted, "qfsoft_db_prmp", sql2);
			if (dtm2.getRowCount() == 0) {
				break;
			}
			for (int i = 0; i < dtm2.getRowCount(); i++) {
				String fund_idv = DealTable.getString(dtm2, i, "fund_id");
				String fund_codev = DealTable.getString(dtm2, i, "fund_code");
				try {
					SeleniumMethod.visit(ted, "http://fund.eastmoney.com/" + fund_codev + ".html");
					JsonObject jsonv1 = new JsonObject();
					DealJson.setValue(jsonv1, "成立日期", SeleniumMethod.getText(ted, ".//div[@class='infoOfFund']/table/tbody/tr[2]/td[1]").replace("成 立 日：", ""));
					DealJson.setValue(jsonv1, "基金经理", SeleniumMethod.getText(ted, ".//div[@class='infoOfFund']/table/tbody/tr[1]/td[3]/a"));
					DealJson.setValue(jsonv1, "基金规模", DealString.getParam(SeleniumMethod.getText(ted, ".//div[@class='infoOfFund']/table/tbody/tr[1]/td[2]"), "基金规模：", "（", 0));
					...
					DealDatabase.set_field(hmv, "fund_value", "value", jsonv1);
					...
					String sqlv = DealDatabase.getModifySql("t_invest_fund", hmv, "fund_id='" + DealDatabase.cvtString(fund_idv) + "'");
					DatabaseMethod.edit(ted, "qfsoft_db_prmp", sqlv);
				} catch (Exception e) {
					FrameworkMethod.log(Level.WARN, "取值错误：fund_code=" + fund_codev);
				}
			}
		}
		return null;
